...
(function spawnParticles() {
  ...
  for (let i = 0; i < 35; i++) {
    ...
    p.style.cssText = `
      ...
      filter:blur(${Math.random()<0.3?0.8:0}px);opacity:0;`;
...
function drawBg(tCtx, W, H, lineH) {
  ...
  if (state.background==='lined') {
    ...
    tCtx.save();
    tCtx.strokeStyle='rgba(220,120,120,0.55)';
    tCtx.lineWidth=1.5;
    tCtx.beginPath(); tCtx.moveTo(68,0); tCtx.lineTo(68,H); tCtx.stroke();
    tCtx.restore();

    tCtx.save();
    tCtx.strokeStyle='rgba(160,195,235,0.6)';
    tCtx.lineWidth=1;
    ...
    tCtx.save();
    tCtx.strokeStyle='rgba(185,165,115,0.28)';
    tCtx.lineWidth=2;
    tCtx.beginPath();
...
function drawChar(tCtx, ch, x, y, charIdx, baseOpacity, [r,g,b]) {
  ...
  let sizeScale = 1;
  if (state.pressureEffect) {
    sizeScale = 1 + (seededRand()-0.5) * 0.04;
  }
...
function renderToCanvas(targetCanvas, targetCtx, logicalW, scale) {
  ...
  lines.forEach((line, li) => {
    ...
    const lineDrift = state.baselineDrift && state.jitter>0
      ? (seededRand()-0.5)*state.jitter*0.6
      : 0;
...
document.getElementById('inputText').addEventListener('input', e => {
  state.text = e.target.value; debouncedDraw();
});
...
inkOpEl.addEventListener('input', () => {
  state.inkOpacity = parseInt(inkOpEl.value)/100;
  inkOpLbl.textContent = inkOpEl.value+'%';
  debouncedDraw();
});
...
jitterEl.addEventListener('input', () => {
  state.jitter = parseFloat(jitterEl.value);
  jitterLbl.textContent = state.jitter===0 ? 'Off' : state.jitter.toFixed(1);
  debouncedDraw();
});
...
document.getElementById('shadowToggle').addEventListener('change', e => {
  state.shadow = e.target.checked; drawCanvas();
});

const inkVarEl  = document.getElementById('inkVariation');
const inkVarLbl = document.getElementById('inkVariationVal');
inkVarEl.addEventListener('input', () => {
  const v = parseFloat(inkVarEl.value);
  state.inkVariation = v;
  inkVarLbl.textContent = v === 0 ? 'Off' : Math.round(v * 100) + '%';
  debouncedDraw();
});

document.getElementById('pressureToggle').addEventListener('change', e => {
  state.pressureEffect = e.target.checked; drawCanvas();
});

document.getElementById('baselineToggle').addEventListener('change', e => {
  state.baselineDrift = e.target.checked; drawCanvas();
});
...
document.querySelectorAll('.bg-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    previewScroll.classList.toggle('checker', state.background==='transparent');
    drawCanvas();
  });
});
